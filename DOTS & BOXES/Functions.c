...
void Colors(int a6[5][12],char C[7][8],int a[7][8],int GHline[],int GVline[],int a5[],int a3[2][4],int aa[4][4],int rr,int score1,int score2,int r,int f,int J,time_t start,time_t finish,int num,int moves,int K,int H){
     ...
     gotoxy(0,2);
    for (i=0; i<11; i++){   //To color framework with player's color in his turn
            if (i==0||i==10){
                    if (num%2==0){
    ...
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),15);
     ...
     printf(" %s:%d ",player2.name,score2);
     SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),15);
     ...
     printf("%s moves:%d  %s moves:%d",player1.name,K,player2.name,H); //K for player 1 , H for player 2
     SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),14);
     gotoxy(70,18);
     printf("INSTRUCTIONS");
     gotoxy(70,19);
     ...
     gotoxy(J+5,f+4); //Print arrow
     SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),13);
     printf("%c",C[f][J]);
     SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),15);

     for (i=0; i<rr; i++){  //Print lines which have played with colors
                if (a6[4][i]%2==0){ //Check if line belong to player 1 or 2
            if (a6[0][i]==a6[1][i]){
                gotoxy(a6[2][i]+5,a6[0][i]+4);
         for(iii=0;iii<4;iii++){ //Printing sample with color
            if((iii==1)||(iii==2)){
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),10);
                printf("%c",GHline[iii]);
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),15);
   ...
   if ((score1!=0)||(score2!=0)){  //Print square with suitable color
        for (i=0; i<r; i++){
            if (a5[i]==1){ //check square if belongs to player 1 or 2
                for(ii=0;ii<4;ii++){ //printing sample with suitable color
...
//Function to calculate score player 1
void S1(int a[7][8],int a3[2][4],int *r,int *score1,int a5[],int *num,int *score2){
 int l,i,t,b;
 for(i=0;i<6;i+=3){ //for loop to check all squares of the game
            l=0;
        for(b=0;b<6;b+=3){
         if((a[i][b+1]==45)&&(a[i+3][b+1]==45)&&(a[i+1][b]==124)&&(a[i+1][b+3]==124)){ //Conditions to be checked
           l=0;
           if ((*score1==0)&&(*score2==0)){ //if any square is closed for the first time
                    *score1=*score1+1;
                    a3[0][*r]=i;  //a3 array that saves coordinates of closed square
                    a3[1][*r]=b;
                    a5[*r]=1;     //a5 to refer closed square to right player
                    *num=*num-1; //To give the player that wins the square additional move
                    *r=*r+1;    //Counter to control size of a3 , a5

                }else {
                for (t=0; t<*r; t++){  //For loop to check if this square has closed before or not
                    if ((i==a3[0][t])&&(b==a3[1][t])){
                        l++;
                    }
                }
                if (l==0){ //Square wasn't closed before
                        a3[0][*r]=i;
                        a3[1][*r]=b;
                        a5[*r]=1;
                        *score1=*score1+1;
...
//Function to calculate score player 2
...
int i,b,l=0,P=0;
if (*rr!=0){
        if (a6[4][*rr-1]%2==0){ //To decrease number of moves for each players
            *K=*K-1; //P1
        }else{
        *H=*H-1; //P2
        }
        k=a6[0][*rr-1];  //Logic of undo depend of decreasing the size of array of movements , returning # to * , | or - to space
        v=a6[1][*rr-1];
        u=a6[2][*rr-1];
        m=a6[3][*rr-1];
        *flag=2;
        *rr=*rr-1;
        *undo=*undo+1; //To know how many times can i do redo
         if(k==v){ //returning
                *moves=*moves+1;
        a[k][m]=a[v][u]=42;
        for(i=k;i<(k+1);i++){
  ...
  for(i=0;i<6;i+=3){ //check if square has opened
            l=0;
        for(b=0;b<6;b+=3){
         if((a[i][b+1]==45)&&(a[i+3][b+1]==45)&&(a[i+1][b]==124)&&(a[i+1][b+3]==124)){
           l=0;
...
int i,b,t,l=0;  //In this function turn all comments of undo to understand ^_^
*flag=2;
             if (*undo==0){ //To check if i can do redo or not
                 gotoxy(0,13);
...
//Function to control the position of cursor
...
void p1Vsp2 (){
    ...
    char C[7][8]={{'*',' ',' ','*',' ',' ','*','<'},   //array to control of arrow
                  {' ',' ',' ',' ',' ',' ',' ',' '},
                  {' ',' ',' ',' ',' ',' ',' ',' '},
                  {'*',' ',' ','*',' ',' ','*',' '},
                  {' ',' ',' ',' ',' ',' ',' ',' '},
    ...
    int aa[4][4]={{'#','-','-','#'},  //sample to print colored square
                  {'|',' ',' ','|'},
                  {'|',' ',' ','|'},
                  {'#','-','-','#'}};
    int a6[5][12];  //array of movements
    int GHline[4]={35,45,45,35};  //sample to print colored lines
    int GVline[4]={35,124,124,35};
    int a3[2][4]={{1,1,1,1},  //To store coordinates of closed squares
                   {1,1,1,1}};
    int a5[4]={0,0,0,0}; //Store flag that refer to who player that closes square
    int ch=0,i,l,b,score1=0,score2=0,x,num,y,v,m,z,n,k,u,q=18,r=0,f=0,temp=0,x1=0,y1=0,flag=0,x2=0,y2=0,undo=0,P=0,f2=0,J=7,rr=0,D,TEMP,moves=12,K=0,H=0;
    ...
    for (num=2; num<20; num++){ //For loop that switch between players

    if ((num%2)==0){ //P1

    Colors(a6,C,a,GHline,GVline,a5,a3,aa,rr,score1,score2,r,f,J,start,finish,num,moves,K,H);

    while (1){ //To control of what user will input
            if (flag==2){
                break;
            }
      ...
      ch=0;
  while (ch==0){ //To keep time dynamic (interrupt)
       finish = clock();
       if (kbhit()){
        ch=getch();
       }
  Sleep(10); //To avoid printing error
     k=(((finish - start)) / CLOCKS_PER_SEC);
     gotoxy(19,7);
     printf("time : %d%d : %d%d                   ",(k/60)/10,(k/60)%10,(k%60)/10,k%10);
     gotoxy(0,1);
  ...
  if ((ch!=13)&&(ch!=117)&&(ch!=114)&&(ch!=115)&&(ch!=27)){ //Checking which button that you have pressed
   //Pressed on one of arrow buttons
  Arrows(ch,a6,C,a,GHline,GVline,a5,a3,aa,rr,score1,score2,r,&f,&J,start,finish,num,moves,K,H);
  }else if (ch==13){ //Pressed enter
  Detect_Check(&flag,J,f,start,finish,&x1,&y1,&x2,&y2,a6,C,a,GHline,GVline,a5,a3,aa,rr,score1,score2,r,num,moves,K,H);
  if (flag==2){
    K++; //Increasing P1 movement
  }
  }else if (ch==117){
         Undo(a6,&rr,&undo,k,v,m,u,a,a3,a5,&r,&score1,&score2,&flag,&f2,&moves,&K,&H);
         if (f2>0){ //To have correct turn if two squares were closed in same movement
            num--;
            f2=0;
         }
        finish = clock();
        Colors(a6,C,a,GHline,GVline,a5,a3,aa,rr,score1,score2,r,f,J,start,finish,num,moves,K,H);
            ...
            Redo(a6,&rr,&undo,k,v,m,u,a,a3,a5,&r,&score1,&score2,&flag,&f2,&moves,&K,&H);
            if (f2>0){
            num--;
            f2=0;
         }
         finish = clock();
        Colors(a6,C,a,GHline,GVline,a5,a3,aa,rr,score1,score2,r,f,J,start,finish,num,moves,K,H);
        }else if(ch==115){ //Pressed S , choose one of slots to save
            gotoxy(40,9);
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),11);
            printf("(1) SAVE1");
            gotoxy(40,11);
            ...
            while((choi!='1') && (choi!='2') && (choi!='3')){
                ...
                printf("                     ");
            if(choi=='1'){
                save1(a,rr,a6,r,a3,C,a5,J,f,score1,score2,finish,K,H,moves);
                Sleep(500);
                gotoxy(40,15);
                printf("                                                    ");
                gotoxy(40,9);
                ...
                save2(a,rr,a6,r,a3,C,a5,J,f,score1,score2,finish,K,H,moves);
                Sleep(500);
                gotoxy(40,15);
                printf("                                                    ");
                gotoxy(40,9);
                ...
                save3(a,rr,a6,r,a3,C,a5,J,f,score1,score2,finish,K,H,moves);
                Sleep(500);
                gotoxy(40,15);
                printf("                                                    ");
                gotoxy(40,9);
...
if (ch==13){
undo = 0;
Drawing(x1,x2,y1,y2,&rr,a,a6,&moves,num); //To draw movement in game's array
    }
    S1(a,a3,&r,&score1,a5,&num,&score2); //To calculate score
    if(moves==0){
        Colors(a6,C,a,GHline,GVline,a5,a3,aa,rr,score1,score2,r,f,J,start,finish,num,moves,K,H);
    ...
    }else { //Next turn , the same of previous comments ^_^
...
if (ch==13){
undo = 0;
  Drawing(x1,x2,y1,y2,&rr,a,a6,&moves,num);
    }
...
void p1Vsp2_load1 (){
   ...
   time_t start=0, finish=0;
start = clock();
   load1(a,&rr,a6,&r,a3,C,a5,&J,&f,&score1,&score2,&finish,&K,&H,&moves);
   finish=clock()+(timee.hhh/2);
   Colors(a6,C,a,GHline,GVline,a5,a3,aa,rr,score1,score2,r,f,J,start,finish,num,moves,K,H);
   ...
   YY=rr;
    ...
    for (num=a6[4][rr-1]+1; num<20; num++){
...
void p1Vsp2_load2 (){
   ...
   time_t start=0, finish=0;
start = clock();
   load2(a,&rr,a6,&r,a3,C,a5,&J,&f,&score1,&score2,&finish,&K,&H,&moves);
   finish=clock()+(timee.hhh/2);
   Colors(a6,C,a,GHline,GVline,a5,a3,aa,rr,score1,score2,r,f,J,start,finish,num,moves,K,H);
...
void p1Vsp2_load3 (){
   ...
   time_t start=0, finish=0;
start = clock();
   load3(a,&rr,a6,&r,a3,C,a5,&J,&f,&score1,&score2,&finish,&K,&H,&moves);
   finish=clock()+(timee.hhh/2);
   Colors(a6,C,a,GHline,GVline,a5,a3,aa,rr,score1,score2,r,f,J,start,finish,num,moves,K,H);
